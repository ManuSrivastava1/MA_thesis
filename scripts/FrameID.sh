...
if [ "${mode}" = train ]; then
  ...
  python3 ${SRC_DIR}/main.py --mode train --home ${XP_DIR}/${xp}/frameid --embedding_name ${embs} --dataset_v fn${fnv}
  echo "Finished the experiment for training the classifier"
else
  #xpv="$2"
  #echo $xpv
  bash ./prepare.sh ${xp}
  ...
  python3 ${SRC_DIR}/main.py --mode test --home ${XP_DIR}/${xp}/frameid --embedding_name ${embs} --dataset_v fn${fnv}
  echo "Finished setup for TESTING the classifier"
fi
