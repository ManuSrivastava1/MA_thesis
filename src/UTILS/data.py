...
def get_graphs(src_sentences, src_fes, verbose=False):  # files in, graphs out
	...
	with codecs.open(src_sentences, "r", "utf-8") as in_sentences:
		with codecs.open(src_fes, "r", "utf-8") as in_fes:
			...
			print(f'File name of sentences      -> {src_sentences.split("/")[-1]}')
			print(f'File name of frame-elements -> {src_fes.split("/")[-1]}')
			print(f'Labeled                     -> {len(srl_data)}')
			print(f'parsed                      -> {len(sentences)}')
			print(f'Number of graphs in the set -> {len(graphs)}')
